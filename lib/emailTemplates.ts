...
export function broadcastEmailHtml(toName: string, messageBody: string, manageUrl?: string): string {
  const safe = esc(messageBody).replace(/\n/g, "<br/>");
  const manageLine = manageUrl
    ? `<p style='font-family:sans-serif;font-size:12px;color:#bbb;margin-top:20px;'><a href='${esc(manageUrl)}' style='color:#bbb;'>Manage your email/text notification preferences</a></p>`
    : "";
  return `<!DOCTYPE html><html><head><meta charset='UTF-8'/><style>${BASE_STYLE}.greeting{font-size:17px;color:#3d2e1e;margin:0 0 20px;}.message{font-family:sans-serif;font-size:14px;color:#5a3e28;line-height:1.8;}</style></head><body><div class='wrap'><div class='header'><h1>🧘 AOP Shala NYC</h1><p>Message from your teacher</p></div><div class='body'><p class='greeting'>Hi ${esc(toName) || "there"},</p><div class='message'>${safe}</div>${manageLine}</div><div class='footer'>AOP Shala NYC · intouchyoga@icloud.com</div></div></body></html>`;
}

export function packageExhaustedEmailHtml(p: { studentName: string; studentEmail: string; totalClasses: number }): string {
  return `<!DOCTYPE html><html><head><meta charset='UTF-8'/><style>${BASE_STYLE}.alert-badge{display:inline-block;background:#fde8e8;color:#c44;font-family:sans-serif;font-size:12px;font-weight:600;padding:4px 12px;border-radius:20px;text-transform:uppercase;letter-spacing:0.5px;margin-bottom:16px;}</style></head><body><div class='wrap'><div class='header'><h1>🧘 AOP Shala NYC</h1><p>Package Alert</p></div><div class='body'><div class='alert-badge'>⚠️ Package Exhausted</div><p class='section-title'>Student</p><div class='card'><div class='row'><div class='row-label'>Name</div><div class='row-value'>${esc(p.studentName)}</div></div><div class='row'><div class='row-label'>Email</div><div class='row-value'>${esc(p.studentEmail)}</div></div></div><p class='section-title'>Package</p><div class='card'><div class='row'><div class='row-label'>Package Size</div><div class='row-value'>${p.totalClasses} classes</div></div><div class='row'><div class='row-label'>Classes Remaining</div><div class='row-value' style='color:#c44;font-weight:700;'>0 — fully used</div></div></div><p style='font-family:sans-serif;font-size:14px;color:#5a3e28;line-height:1.7;'>This student just used their last class. Consider reaching out to renew their package!</p></div><div class='footer'>AOP Shala NYC · intouchyoga@icloud.com</div></div></body></html>`;
}
